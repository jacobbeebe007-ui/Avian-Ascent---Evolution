// js/combat/enemy-damage.js — boss burst cap + enemy-side damage rolls. Load after js/core/game.js.
...
function applyBossBurstBuffer(rawDamage) {
  ...
  const cap = Math.floor(maxHp * 0.4);
  if (dmg <= cap) return dmg;
  const excess = dmg - cap;
  return cap + Math.floor(excess * 0.7);
}
...
function edmg(mult = 1) {
  ...
  const ruffleReduct =
    G.enemyStatus.featherRuffle && G.enemyStatus.featherRuffle.turns > 0
      ? 1 - (G.enemyStatus.featherRuffle.atkReduction || 0) / 100
      : 1;
  let out = Math.floor(roll(Math.floor(b * 0.8), Math.floor(b * 1.2)) * mult * lull * weak * ruffleReduct);
